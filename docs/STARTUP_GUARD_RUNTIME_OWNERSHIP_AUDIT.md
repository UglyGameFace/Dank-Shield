...
## Retired interaction action lock patcher

`interaction_action_lock_guard.py` is already absent from the repository and has no production boot owner. Native duplicate-action ownership lives in `interaction_guard.py`, and `tests/test_interaction_native_ownership.py` requires the retired module to stay absent while proving native import leaves `discord.ui.View._scheduled_task` unchanged.

The earlier audit row describing this module as a direct `main.py` import was stale and is corrected here.
