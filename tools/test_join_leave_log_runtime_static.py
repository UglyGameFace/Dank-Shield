...
def test_runtime_bootstrap_calls_authoritative_router_install() -> None:
    assert "member_lifecycle_router_guard" in RUNTIME_BOOTSTRAP
    assert "router.install()" in RUNTIME_BOOTSTRAP
...
def test_setup_logs_uses_explicit_leave_route() -> None:
...
def test_canonical_runtime_owns_public_join_output() -> None:
    assert "send_live_welcome_card" in CARD_RUNTIME
    assert "welcome_cards_enabled(cfg)" in CARD_RUNTIME
    assert "welcome_join_enabled" not in CARD_RUNTIME
