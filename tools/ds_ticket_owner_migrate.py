...
p = Path('stoney_verify/commands_ext/public_ticket_panel_clean.py')
text = p.read_text(encoding='utf-8')

if 'import time\nimport uuid\n' not in text:
    anchor = 'import re\n'
    if anchor not in text:
        raise SystemExit('ticket imports anchor missing')
    text = text.replace(anchor, anchor + 'import time\nimport uuid\n', 1)

globals_anchor = '_NUMBER_LOCKS: Dict[int, asyncio.Lock] = {}\n'
globals_block = '''_NUMBER_LOCKS: Dict[int, asyncio.Lock] = {}\n_PANEL_INTERACTION_LOCKS: Dict[int, asyncio.Lock] = {}\n_PANEL_INTERACTION_DONE_UNTIL: Dict[int, float] = {}\n_MENU_SESSIONS: Dict[Tuple[int, int], Dict[str, Any]] = {}\n_CONFIRM_LOCKS: Dict[Tuple[int, int], asyncio.Lock] = {}\n_MENU_TTL_SECONDS = 900.0\n_INTERACTION_TTL_SECONDS = 90.0\n'''
if '_MENU_SESSIONS:' not in text:
    if globals_anchor not in text:
        raise SystemExit('ticket globals anchor missing')
    text = text.replace(globals_anchor, globals_block, 1)

start = text.find('class TicketConfirmView(discord.ui.View):\n')
end = text.find('\n\nasync def handle_public_ticket_panel_click(', start)
if start < 0 or end < 0:
    if 'async def _handle_panel_button_core' not in text:
        raise SystemExit('ticket view replacement markers missing')
else:
    replacement = r'''def _session_key(guild_id: int, user_id: int) -> Tuple[int, int]:
...
for line in (
    ...
    init_text = init_text.replace(line, '')
init_text = init_text.replace('    "✅ public_ticket_confirm_hardening_guard",\n', '')
init.write_text(init_text, encoding='utf-8')

print('ticket owner migration complete')
