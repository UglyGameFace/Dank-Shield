...
-- Safe to rerun; custom owner categories remain untouched.
-- ============================================================
...
select case
    when lower(coalesce(p_slug,'')) in ('verification','verification-help','verification_issue','verification-issue','verify')
      or lower(coalesce(p_name,'')) like '%verification%' then 'verification'
    when lower(coalesce(p_slug,'')) in ('bug','bug-report','technical_support','technical-support','bug-technical-support')
      ...
      or lower(coalesce(p_name,'')) like '%bug%' then 'bug'
    when lower(coalesce(p_slug,'')) in ('custom','other','general','general-support','support')
      or lower(coalesce(p_name,'')) in ('other','general','general support','support') then 'support'
    when lower(coalesce(p_slug,'')) in ('cod_services','cod-services')
      or lower(coalesce(p_name,'')) like '%call of duty%'
      or lower(coalesce(p_name,'')) like '%cod services%' then 'cod-services'
    when lower(coalesce(p_slug,'')) in ('account_access','account-access') then 'account-access'
    when lower(coalesce(p_slug,'')) in ('payments_refunds','payments-refunds') then 'payments-refunds'
    when lower(coalesce(p_slug,'')) in ('staff_complaint','staff-complaint') then 'staff-complaint'
    when lower(coalesce(p_slug,'')) in ('service_request','service-request') then 'service-request'
    when lower(coalesce(p_slug,'')) in ('vouch_referral','vouch-referral') then 'vouch-referral'
    when lower(coalesce(p_slug,'')) in ('giveaway_reward','giveaway-reward') then 'giveaway-reward'
    when lower(coalesce(p_slug,'')) in ('content_media','content-media') then 'content-media'
...
begin
    for g in
        select distinct x.guild_id from (
            select tc.guild_id::text guild_id from public.ticket_categories tc
            union
            select gc.guild_id::text guild_id from public.guild_configs gc
        ) x
        where p_guild_id is null or x.guild_id=p_guild_id
    loop
        inserted_n:=0; updated_n:=0; deleted_n:=0;
        ...
        for c in select * from public.dank_ticket_category_catalog() loop
            select tc.id::text into winner_id
              from public.ticket_categories tc
             where tc.guild_id::text=g.guild_id
               and public.dank_ticket_category_key(tc.slug,tc.name)=c.category_key
             order by case when tc.managed_by_dank then 0 else 1 end,
                      case when tc.slug=c.slug then 0 else 1 end,
                      coalesce(tc.sort_order,9999), tc.created_at nulls last, tc.id
             limit 1;
            ...
            if winner_id is not null then
                ...
                update public.ticket_categories
                   set slug=c.slug,name=c.name,description=c.description,
                       intake_type=c.intake_type,match_keywords=c.match_keywords,
                       sort_order=c.sort_order,is_default=c.is_default,
                       managed_by_dank=true,managed_catalog_version=c.catalog_version,
                       managed_category_key=c.category_key,updated_at=now()
                 where id::text=winner_id
                   and (slug, name, description, intake_type, match_keywords, sort_order,
                        is_default, managed_by_dank, managed_catalog_version, managed_category_key)
                       is distinct from
                       (c.slug,c.name,c.description,c.intake_type,c.match_keywords,c.sort_order,
                        c.is_default,true,c.catalog_version,c.category_key);
                get diagnostics affected=row_count;
                ...
                insert into public.ticket_categories(
                    ...
                    sort_order,is_default,managed_by_dank,managed_catalog_version,managed_category_key
                ) values (
                    g.guild_id,c.slug,c.name,c.description,c.intake_type,c.match_keywords,
                    c.sort_order,c.is_default,true,c.catalog_version,c.category_key
                );
        ...
        update public.ticket_categories
         ...
         where guild_id::text=g.guild_id and managed_by_dank=true;
