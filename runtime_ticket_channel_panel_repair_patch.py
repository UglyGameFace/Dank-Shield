...
Problem this fixes:
A ticket channel can exist without the in-channel staff/user controls if the bot
created the channel while Discord overwrites blocked Send Messages / View Channel
or if a channel was manually moved after creation. Moving the channel later does
not recreate the missing bot message.

This guard:
- posts/repairs the in-ticket controls after ticket creation
- sweeps existing open ticket channels on ready
- does not spam duplicate panels if one already exists
"""
...
def _log(message: str) -> None:
    try:
        print(f"🧰 runtime_ticket_channel_panel_repair {message}")
    except Exception:
        pass
...
def _warn(message: str) -> None:
    try:
        print(f"⚠️ runtime_ticket_channel_panel_repair {message}")
    except Exception:
        pass
...
async def _has_existing_control_message(channel: discord.TextChannel) -> bool:
    try:
        ...
        _warn(f"cannot scan ticket controls channel={channel.id} reason=Missing Access missing={_permission_problem(channel)} raw={e!r}")
        return True  # avoid duplicate attempts when we cannot inspect
    except Exception as e:
...
async def ensure_ticket_channel_panel(channel: discord.TextChannel, *, reason: str = "repair") -> bool:
    ...
    try:
        ...
        _warn(f"cannot post ticket control panel channel={channel.id} reason=Missing Access missing={_permission_problem(channel)} raw={e!r}")
    except Exception as e:
