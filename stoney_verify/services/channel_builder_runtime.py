...
def _math_letters(prefix: str, *, digit_prefix: str | None = None, special: dict[str, str] | None = None) -> dict[str, str]:
    ...
    for index, ch in enumerate("ABCDEFGHIJKLMNOPQRSTUVWXYZ"):
        looked = special.get(ch) or _lookup(f"MATHEMATICAL {prefix} CAPITAL {ch}", None)
    ...
    for index, ch in enumerate("abcdefghijklmnopqrstuvwxyz"):
        looked = special.get(ch) or _lookup(f"MATHEMATICAL {prefix} SMALL {ch.upper()}", None)
        if looked and looked != ch:
            out[ch] = looked
        elif small_base:
    ...
    if digit_prefix:
        ...
        for index, (digit, word) in enumerate((
            ("0", "ZERO"),
            ("1", "ONE"),
            ("2", "TWO"),
            ("3", "THREE"),
            ...
            looked = _lookup(f"MATHEMATICAL {digit_prefix} DIGIT {word}", None)
