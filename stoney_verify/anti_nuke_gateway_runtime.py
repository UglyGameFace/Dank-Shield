...
The broad implementation lives in ``anti_nuke_guardian_runtime``.  This module
keeps the historical installer and role-create fast path so production boot and
...
async def _contain_actor(guild: discord.Guild, actor: Any, *, reason: str):
    ...
    async with lock:
        return await anti_nuke._contain_actor(guild, actor, reason=reason)  # noqa: SLF001
...
def install_anti_nuke_gateway_runtime(bot: discord.Client) -> bool:
    """Install exactly one guardian-backed audit listener on the production bot."""
    ...
    if moderation:
        print("🛡️ AntiNuke guardian gateway active with broad audit coverage")
    else:
        print(
