...
"""
Native ticket sync/backfill category guard.

This replaces the old root-level runtime_ticket_sync_native_patch.py.

This is intentionally conservative: it wires sync_service into the existing
sync_categories helpers without rewriting the large sync_service file in one risky
pass. That keeps startup/backfill stable while moving the behavior out of the
project root and into the ticket package where it belongs.
"""
...
def _log(message: str) -> None:
    try:
        print(f"🧩 ticket_sync_native_guard {message}")
    except Exception:
        pass
...
def _warn(message: str) -> None:
    try:
        print(f"⚠️ ticket_sync_native_guard {message}")
    except Exception:
        pass


def _native_sync_categories() -> Optional[Any]:
    """Return sync_categories lazily without import-hook recursion."""
    ...
    try:
        ...
        _warn(f"sync_categories import hit recursion and was disabled for this boot: {e!r}")
    except Exception as e:
        _NATIVE_IMPORT_FAILED = True
        _warn(f"sync_categories import failed and was disabled for this boot: {e!r}")
    return None
...
def _patch_sync_service(module: Any) -> None:
    ...
    try:
        ...
        if patched_any:
            _PATCHED.add(key)
            _log(f"patched {module_name}; startup ticket sync/backfill uses native sync category helpers")
    finally:
        _PATCHING = False
...
_patch_loaded()
_log("loaded; native ticket sync/backfill category guard active")
