...
"""
Active ticket category enforcer.

This replaces the old root-level runtime_ticket_category_enforcer_patch.py.

It is an emergency repair net for open ticket channels that somehow end up
outside the configured Active Tickets category. It lives inside tickets_new so
category enforcement stays with ticket code instead of cluttering the project
root.
"""
...
def _log(message: str) -> None:
    try:
        print(f"🎯 ticket_category_enforcer {message}")
    except Exception:
        pass
...
def _warn(message: str) -> None:
    try:
        print(f"⚠️ ticket_category_enforcer {message}")
    except Exception:
        pass
...
def _channel_in_category(channel: discord.TextChannel, category: discord.CategoryChannel) -> bool:
    try:
        from .category_resolver import channel_is_in_category
...
async def move_ticket_channel_to_active(channel: discord.TextChannel, *, reason_suffix: str = "repair") -> bool:
...
async def _enforce_after_create(guild: Optional[discord.Guild], result: Any, *, started_at: float) -> None:
    ...
    if not _is_open_ticket_channel(channel):
        return
    await move_ticket_channel_to_active(channel, reason_suffix="post-create placement")
...
async def _sweep_guild_open_tickets(guild: discord.Guild) -> tuple[int, int]:
    ...
    for channel in list(getattr(guild, "text_channels", []) or []):
        ...
        if _channel_in_category(channel, category):
            continue
        if await move_ticket_channel_to_active(channel, reason_suffix="startup open-ticket repair"):
            moved += 1
    return checked, moved
...
def _patch_ticket_service(module: Any) -> None:
    ...
    try:
        ...
        _log(f"patched {module_name}; active-ticket category repair uses native resolver")
    finally:
        _PATCHING = False
...
_patch_loaded_once()
_log("loaded; active ticket category repair enabled")


__all__ = ["move_ticket_channel_to_active"]
