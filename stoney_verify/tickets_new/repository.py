...
# ------------------------------------------------------------
# Schema-locked repository layer
#
# Canonical expectations after schema lock:
# - tickets includes the informative compatibility columns
# - ticket_notes is the canonical notes table
...
_ALLOWED_TICKET_WRITE_COLUMNS: Set[str] = (
    _BASE_TICKET_WRITE_COLUMNS | _EXTENDED_TICKET_WRITE_COLUMNS
)
...
def _normalize_activity_feed_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    meta = _safe_meta(row.get("meta") or metadata)
...
def build_ticket_payload(
    *,
    ...
    payload: Dict[str, Any] = {
        ...
        "mod_suggestion_confidence": mod_suggestion_confidence if mod_suggestion_confidence is not None else 0,
        "created_at": now_iso,
        "updated_at": now_iso,
        "closed_at": None,
...
async def insert_ticket(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        ...
        res = await _insert_ticket_async(clean)
        rows = _result_rows(res)
        if rows:
            return _normalize_ticket_row(rows[0])
    except Exception as e:
...
async def upsert_ticket(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if channel_id:
            try:
                res = await _upsert_ticket_async(clean)
                rows = _result_rows(res)
                if rows:
                    return _normalize_ticket_row(rows[0])
        ...
        if existing and existing.get("id") is not None:
            ...
            res = await _update_ticket_by_id_async(ticket_id, patch)
            rows = _result_rows(res)
            if rows:
                return _normalize_ticket_row(rows[0])
...
async def update_ticket_by_channel_id(
    ...
    *,
    ...
    try:
        ...
        writer = (
            _update_ticket_by_any_channel_id_async
            if allow_thread_fallback
    ...
    *,
    ...
    try:
        def _insert_ticket_note_sync():
            return sb.table(TICKET_NOTES_TABLE).insert(payload).execute()
    ...
    *,
    ...
    try:
        def _insert_event_sync():
            return sb.table(ACTIVITY_FEED_EVENTS_TABLE).insert(payload).execute()
    ...
    *,
    ...
    try:
        def _insert_join_sync():
            return sb.table(MEMBER_JOINS_TABLE).insert(payload).execute()
...
async def repository_healthcheck() -> Dict[str, Any]:
    out: Dict[str, Any] = {
        ...
        "notes_backend": TICKET_NOTES_TABLE,
        "schema_locked_ticket_columns": sorted(_EXTENDED_TICKET_WRITE_COLUMNS),
    }
    ...
    try:
        ...
        def _probe_notes():
            return sb.table(TICKET_NOTES_TABLE).select("*").limit(1).execute()

        def _probe_activity():
            return sb.table(ACTIVITY_FEED_EVENTS_TABLE).select("*").limit(1).execute()

        await _run_db_op("tickets repository healthcheck", _probe_tickets)
        await _run_db_op("ticket notes repository healthcheck", _probe_notes)
        await _run_db_op("activity feed repository healthcheck", _probe_activity)
