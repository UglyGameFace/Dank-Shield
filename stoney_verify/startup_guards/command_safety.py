...
"""
Command registration safety guard.

This replaces the old root-level runtime_command_safety.py.

It keeps startup alive if Discord's global slash-command cap is hit and logs the
command budget before/after sync. It also loads the package-local auto-shard and
global-sync guards first.
"""
...
try:
    ...
    try:
        print(f"⚠️ command_safety failed to import auto_shard guard: {e!r}")
    except Exception:
        pass

try:
    from . import global_command_sync  # noqa: F401
except Exception as e:
    try:
        print(f"⚠️ command_safety failed to import global_command_sync guard: {e!r}")
    except Exception:
        pass
...
def _log(message: str) -> None:
    try:
        print(f"🧷 command_safety {message}")
    except Exception:
        pass
...
def _warn(message: str) -> None:
    try:
        print(f"⚠️ command_safety {message}")
    except Exception:
        pass
...
def _log_command_budget(tree: Any, *, force: bool = False, reason: str = "") -> None:
    ...
    if force or count >= WARN_AT or skipped > 0:
        ...
        try:
            print(
                f"{level} command_safety command budget "
...
def _install_budget_logger_on_sync(app_commands: Any) -> None:
    ...
    if getattr(original_sync, "_command_budget_wrapped", False) or getattr(original_sync, "_runtime_command_budget_wrapped", False):
        return
    ...
    try:
        setattr(_safe_sync, "_command_budget_wrapped", True)
        setattr(_safe_sync, "_command_budget_original", original_sync)
    except Exception:
        pass
...
def install_command_limit_guard() -> None:
    ...
    if getattr(original_add_command, "_command_safety_wrapped", False) or getattr(original_add_command, "_runtime_command_safety_wrapped", False):
        _PATCHED = True
    ...
    try:
        setattr(_safe_add_command, "_command_safety_wrapped", True)
        setattr(_safe_add_command, "_command_safety_original", original_add_command)
    except Exception:
        pass
