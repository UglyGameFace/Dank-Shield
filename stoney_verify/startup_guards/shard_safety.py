...
"""
Shard/scale readiness guard.

This replaces the old root-level runtime_shard_safety.py.

It does not change the bot class. It adds guild/shard pressure logging and
...
def _log(message: str) -> None:
    try:
        print(f"🛰️ shard_safety {message}")
    except Exception:
        pass
...
def _log_snapshot(bot: Any, *, reason: str, force: bool = False) -> None:
    ...
    if guild_count >= critical_at:
        ...
        _warn(message + " status=critical; migrate to AutoShardedBot/multi-process shards before adding more servers")
    ...
    if bot_class == "Bot" and guild_count >= warn_at:
        _warn("regular commands.Bot is still active at high guild count. Next architecture step: env-gated AutoShardedBot switch.")
...
def _patch_globals(module: Any) -> None:
    ...
    try:
        @bot.listen("on_ready")
        async def _shard_safety_on_ready() -> None:
            try:
            ...
            try:
                task = getattr(bot, "_shard_safety_logger_task", None)
                if task is None or task.done():
                    task = asyncio.create_task(_periodic_shard_scale_logger(), name="shard-scale-logger")
                    setattr(bot, "_shard_safety_logger_task", task)
            except Exception as e:
