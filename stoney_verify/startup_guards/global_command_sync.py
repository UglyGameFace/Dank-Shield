...
"""
Global slash-command sync guard.

This replaces the old root-level runtime_global_command_sync_guard.py.

It blocks accidental large global slash syncs unless explicitly allowed. Guild
syncs still work. Zero-command global syncs still work for migration/clearing.
"""
...
def _log(message: str) -> None:
    try:
        print(f"🌍 global_command_sync_guard {message}")
    except Exception:
        pass
...
def _warn(message: str) -> None:
    try:
        print(f"⚠️ global_command_sync_guard {message}")
    except Exception:
        pass
...
def install_global_command_sync_guard() -> None:
    ...
    if getattr(original_sync, "_global_command_sync_guard_wrapped", False) or getattr(original_sync, "_runtime_global_command_sync_guard_wrapped", False):
        _PATCHED = True
        return
    ...
    try:
        setattr(_guarded_sync, "_global_command_sync_guard_wrapped", True)
        setattr(_guarded_sync, "_global_command_sync_guard_original", original_sync)
    except Exception:
        pass
