...
"""
RaidGuard bot-pattern heuristic hardening.

This replaces the old root-level runtime_raidguard_bot_heuristics_patch.py.

This guard is intentionally conservative:
- improves review signals for disposable/bot-farm-looking accounts
- avoids calling ordinary first-name + birth-year usernames suspicious
- separates heuristic suspicion from hard proof
- adjusts scores with calibrated floors instead of blindly inflating numbers

It does not auto-ban by itself. It makes staff-facing logs more honest.
"""
...
def _log(message: str) -> None:
    try:
        print(f"🧪 raidguard_bot_heuristics {message}")
    except Exception:
        pass
