...
def register_identity_admin_commands(bot: Any, tree: Any) -> None:
    ...
    async def identity_truth(interaction: discord.Interaction, member: discord.Member):
        ...
        if callable(build_member_risk_profile):
            try:
                risk_profile = build_member_risk_profile(member)
            except Exception:
                risk_profile = None

        embed = _render_truth_context_embed(interaction.guild, member, truth, risk_profile)
    ...
    async def identity_confirm_duplicate(
        ...
        if int(member_a.id) == int(member_b.id):
            await reply_once(
                interaction,
        ...
        embed.add_field(name="Member A", value=_member_label(member_a), inline=False)
        embed.add_field(name="Member B", value=_member_label(member_b), inline=False)
    ...
    @app_commands.describe(
        member="Member to attach the trusted fingerprint to",
