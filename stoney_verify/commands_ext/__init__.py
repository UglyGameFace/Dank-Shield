...
_PUBLIC_CORE_MODULES: Tuple[str, ...] = (
    ...
    "public_spam_cleanup_hardening", "public_setup_solid", "public_setup_recommend", "public_setup_recovery", "public_setup_cleanup",
    "public_status_reporter", "public_modlog_coverage", "public_setup_group", "public_help_group",
    "public_cleanup_group", "public_spam_group", "public_mod_group", "public_ticket_group_clean",
    "public_ticket_delete", "public_tickets_group", "public_ticket_intake_group", "public_ticket_category_group",
    "public_tickettool_parity_polish", "public_verify_group", "public_setup_gate",
)
...
def _env_csv_set(name: str) -> set[str]:
    try: return _csv_set(os.getenv(name, "") or "")
    except Exception: return set()

def _env_bool(name: str, default: bool = False) -> bool:
    try:
        raw = os.getenv(name, "")
        if raw is None or str(raw).strip() == "": return bool(default)
...
def _deployment_mode() -> str:
    ...
    if raw: return raw
    if _env_bool("STONEY_PRODUCTION_MODE", False): return "production"
    if _env_bool("STONEY_PUBLIC_MODE", False): return "public"
    return "development"
...
def _selected_command_modules() -> List[CommandModuleSpec]:
    ...
    for label, values in (("STONEY_COMMAND_MODULES", explicit), ("STONEY_COMMAND_MODULES_EXTRA", extra), ("STONEY_COMMAND_MODULES_SKIP", skip)):
        ...
        if unknown: print(f"⚠️ commands_ext: unknown {label} ignored: {unknown}")
    return [spec for spec in COMMAND_MODULES if spec[0] in selected]

def _tree_command_counts(tree: Any) -> tuple[int, int]:
    try: global_count = len(list(tree.get_commands(guild=None) or []))
    except Exception: global_count = len(getattr(tree, "_global_commands", {}) or {})
    guild_count = 0
    try:
        for value in (getattr(tree, "_guild_commands", {}) or {}).values(): guild_count += len(value or {})
    except Exception: pass
    return int(global_count), int(guild_count)

def _child_names(group: Any) -> list[str]:
    try: return sorted(str(getattr(cmd, "name", "")) for cmd in list(getattr(group, "commands", []) or []) if str(getattr(cmd, "name", "")).strip())
    ...
    for name in _STALE_TOP_LEVEL_COMMANDS:
        try:
            if tree.get_command(name, guild=None) is not None:
                tree.remove_command(name, guild=None); removed.append(name)
        except Exception: pass
    if removed: print(f"🧹 commands_ext removed stale top-level commands reason={reason}: {removed}")
    return removed

def _prune_public_stoney_children(*, profile: str, reason: str) -> list[str]:
    if not _public_profile_like(profile): return []
    try: from .public_setup_group import stoney_group
    except Exception: return []
    before = _child_names(stoney_group); removed: list[str] = []
    for name in _CONFUSING_STONEY_CHILDREN:
        try:
            if stoney_group.get_command(name) is not None:
                stoney_group.remove_command(name); removed.append(name)
        except Exception: pass
...
def _import_registrar(module_name: str, function_name: str) -> CommandRegistrar:
    ...
    if not callable(registrar): raise RuntimeError(f"{module_name}.{function_name} is not callable")
    return registrar
...
def _log_stoney_setup_surface() -> None:
    ...
    except Exception as e: print(f"⚠️ commands_ext could not inspect /stoney setup surface: {repr(e)}")

def _public_guard_findings(profile: str) -> tuple[list[str], list[str]]:
    blockers: list[str] = []; warnings: list[str] = []
    deployment = _deployment_mode(); require_auth = _env_bool("BOT_API_REQUIRE_AUTH", True); allow_insecure = _env_bool("BOT_API_ALLOW_INSECURE", False)
    secret = _env_str("BOT_API_SHARED_SECRET", ""); host = _env_str("BOT_API_BIND_HOST", "127.0.0.1"); expected_guilds = _env_int("STONEY_EXPECTED_PUBLIC_GUILDS", 1)
    if profile in {"full", "dev"}: (blockers if deployment in {"public", "prod", "production"} else warnings).append(f"STONEY_COMMAND_PROFILE={profile!r} exposes the old top-level command surface.")
    if not require_auth: (blockers if deployment in {"public", "prod", "production"} else warnings).append("BOT_API_REQUIRE_AUTH=false leaves the structured bot API unauthenticated.")
    if allow_insecure: (blockers if deployment in {"public", "prod", "production"} else warnings).append("BOT_API_ALLOW_INSECURE=true is local-dev only and must be false for public use.")
    if require_auth and len(secret) < 32: (blockers if deployment in {"public", "prod", "production"} else warnings).append("BOT_API_SHARED_SECRET should be at least 32 characters.")
    if host in {"0.0.0.0", "::"} and not require_auth: blockers.append("BOT_API_BIND_HOST is public-facing while API auth is disabled.")
    if expected_guilds >= 100 and not _env_bool("DISCORD_AUTO_SHARD", False): warnings.append("STONEY_EXPECTED_PUBLIC_GUILDS is 100+ but DISCORD_AUTO_SHARD is not enabled.")
    if _env_bool("CLEAR_GLOBAL_COMMANDS_ON_BOOT", False): warnings.append("CLEAR_GLOBAL_COMMANDS_ON_BOOT=true is legacy and ignored in public mode.")
    if _env_str("GUILD_ID", ""): warnings.append("GUILD_ID is still set. Production behavior should rely on per-guild DB config.")
...
def _run_public_startup_guard(profile: str) -> None:
    ...
    for item in blockers: print(f"🚫 public_startup_guard blocker: {item}")
    for item in warnings: print(f"⚠️ public_startup_guard warning: {item}")
    if strict and blockers: raise RuntimeError(f"Public startup guard blocked unsafe deployment: {' | '.join(blockers)}")

def register_all_commands(bot: Any, tree: Any) -> None:
    global _COMMANDS_EXT_REGISTERED
    if _COMMANDS_EXT_REGISTERED:
        print("ℹ️ commands_ext.register_all_commands already ran; skipping duplicate registration."); return
    errors: list[str] = []; profile = _command_profile(); _run_public_startup_guard(profile)
    _remove_stale_top_level_commands(tree, reason="before_module_registration")
    selected_modules = _selected_command_modules(); selected_names = [name for name, _fn, _label in selected_modules]
    ...
    _log_stoney_setup_surface(); _remove_stale_top_level_commands(tree, reason="after_module_registration"); _prune_public_stoney_children(profile=profile, reason="after_module_registration")
    _COMMANDS_EXT_REGISTERED = True
    final_global, final_guild = _tree_command_counts(tree)
    if final_global >= 95: print(f"⚠️ commands_ext command budget high: global={final_global}/100.")
    if errors:
    ...
    else: print(f"✅ commands_ext registration complete. final_global={final_global} final_guild={final_guild} profile={profile}")
