...
"""
One-click default setup for fresh public guilds.
    ...
    *,
    ...
    if category is not None:
        ...
        if apply_channel_permissions and overwrites is not None:
            try:
                await category.edit(overwrites=overwrites, reason="Default bot setup permission refresh")
            except Exception as e:
    ...
    *,
    ...
    if channel is not None:
        ...
        if apply_channel_permissions:
            try:
                kwargs: dict[str, Any] = {"reason": "Default bot setup permission refresh"}
...
async def _assign_control_role_to_runner(
    ...
    *,
    ...
    try:
        await interaction.user.add_roles(control_role, reason="Default bot setup bootstrap")
        ok.append(f"Assigned {control_role.mention} to you for future bot setup access.")
    except Exception as e:
...
async def _setup_defaults_callback(
    ...
    if control_role is None:
        control_role = await _resolve_existing_control_role(guild)
    ...
    if staff_role is None:
        ...
        if existing_staff_id > 0:
            staff_role = guild.get_role(existing_staff_id)
    ...
    blockers = [f"Missing {label}." for label, value in required_pairs if value is None]
    ...
    updates = {
        ...
        "resident_role_id": _role_value(member_role),
...
def _attach() -> None:
    ...
    command = discord.app_commands.Command(
        name="setup-defaults",
        description="Auto-create professional default roles/channels/categories for a fresh server.",
