...
def _build_data_notes_embed(report: InactiveScanReport) -> discord.Embed:
    ...
    embed.add_field(
        name="What This Means",
        value=_safe_field(
            "Confidence now uses calibrated rules: High requires direct member activity evidence; Medium is purge-safe review evidence; Low is shown for manual review but is not purge-safe. Mod-log embeds are scanned too, and reusable evidence is saved if the optional ledger table exists."
        ),
        inline=False,
    )
    return embed
...
def _format_candidate_evidence(candidate: InactiveMemberCandidate, *, limit: int = 900) -> str:
    signals = list(getattr(candidate, "signals", []) or [])
    if not signals:
        return "No direct evidence rows were attached to this candidate."

    signals.sort(key=lambda s: getattr(s, "timestamp", None) or 0, reverse=True)
    lines: list[str] = []
    for signal in signals[:8]:
        timestamp = _fmt_ts(getattr(signal, "timestamp", None), fallback="unknown time")
        source = _trim(str(getattr(signal, "source", "unknown") or "unknown"), 80)
        confidence = str(getattr(signal, "confidence", "?") or "?")
        note = _trim(str(getattr(signal, "note", "") or ""), 150)
        lines.append(f"• **{confidence}** • {timestamp} • `{source}`\n  {note}")
    return _safe_field("\n".join(lines), limit)
...
def _build_user_detail_embed(candidate: InactiveMemberCandidate, *, locked: bool = False) -> discord.Embed:
    ...
    embed.add_field(name="Why This User Was Found", value=_safe_field(candidate.short_reason(700)), inline=False)
    embed.add_field(name="Evidence Used", value=_format_candidate_evidence(candidate), inline=False)
    embed.add_field(
        name="Purge Safety",
        value=_safe_field(
            "✅ Purge-safe" if getattr(candidate, "removable", False) else "⚠️ Manual-only. This user should not be included in purge-all unless reviewed and confirmed."
        ),
        inline=False,
    )
    embed.add_field(
...
class MemberActivityReviewView(discord.ui.View):
    ...
    async def _explain_safety(self, interaction: discord.Interaction) -> None:
        text = (
            ...
            "For verified/resident members, Dank Shield tries to find when the role was granted from its own records first. If that is missing, it can use Discord audit log as a fallback.\n\n"
            "Normal verified/member/resident roles are **not** treated as protected by default. They are the group being reviewed.\n\n"
            "Use **Lock / Skip in Scans** after manually reviewing a user who should not appear again. Low-confidence users are manual-only and should not be purge-all eligible. Use **Locked Users** on this dashboard to review, unlock, or relock skipped users."
        )
        await reply_once(interaction, {"content": text, "ephemeral": True})
