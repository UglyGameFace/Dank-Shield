...
payload. Dank Shield previously exposed whole feature trees under ``/dank`` and
crossed Discord's 8,000-character group limit. This module keeps a few obvious
entry commands and moves the full feature set into buttons, selects, and modals.

This is normal application-command registration, not a runtime method patch.
The original callbacks remain available to the UI and internal services.
"""
...
async def open_dank_home(interaction: discord.Interaction) -> None:
    embed = discord.Embed(
        ...
        description=(
            "Choose what you want to manage. The slash-command list stays short and readable; "
            "the full tools live in guided menus like a real control panel."
        ),
    ...
    embed.add_field(
        ...
        value=(
            "⚙️ Setup • 🛡️ Protection • 👋 Welcome & Join • 👥 Members • 🎨 Design • "
            "🎭 Roles & Profiles • 🧾 Logs • 🩺 Diagnostics"
        ),
        inline=False,
    )
    embed.add_field(
        ...
        value=(
            "Buttons show only the next useful choices, mobile screens stay uncluttered, and advanced settings "
            "no longer flood Discord autocomplete."
        ),
        inline=False,
    )
    embed.set_footer(text="Dank Shield • UI-first command center")
    await _private(interaction, embed=embed, view=DankHomeView(owner_id=interaction.user.id))
...
async def open_help_entry(interaction: discord.Interaction) -> None:
    embed = discord.Embed(
        ...
        description=(
            "Start with `/dank home`. It opens every Dank Shield area without making you memorize dozens of commands."
        ),
        color=discord.Color.blurple(),
    )
    embed.add_field(
    ...
    embed.add_field(
        name="Welcome uploads",
        value=(
            "Use `/dank welcome card-upload` for custom join-card artwork and "
            "`/dank welcome card-font-upload` for a licensed custom font. Everything else is in **Welcome & Join**."
        ),
        inline=False,
    )
    embed.add_field(
    ...
    await _private(interaction, embed=embed, view=DankHelpView(owner_id=interaction.user.id))
...
class _OwnedView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if int(interaction.user.id) == self.owner_id:
            return True
        await _private(interaction, content="❌ Open your own `/dank home` panel to use these controls.")
        return False
...
class DankHomeView(_OwnedView):
    ...
    async def protection(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if not _admin_or_manage(interaction):
            return await _private(interaction, content="❌ Protection settings require **Manage Server** or **Administrator**.")
    ...
    @discord.ui.button(label="Welcome & Join", emoji="👋", style=discord.ButtonStyle.primary, row=0)
    ...
    async def design(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        _ = button
        if not _admin_or_manage(interaction):
            return await _private(interaction, content="❌ Server Design requires **Manage Server** or **Administrator**.")
    ...
    async def logs(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        _ = button
        if not _admin_or_manage(interaction):
            return await _private(interaction, content="❌ Log settings require **Manage Server** or **Administrator**.")
...
class DankHelpView(_OwnedView):
    @discord.ui.button(label="Open Control Center", emoji="🛡️", style=discord.ButtonStyle.primary)
...
def _welcome_upload_group() -> app_commands.Group:
    ...
    from .public_welcome_card_studio import welcome_card_font_clear, welcome_card_font_upload
    from stoney_verify.welcome_setup_ui import open_welcome_setup

    group = app_commands.Group(
        name="welcome",
        description="Open Welcome & Join or upload advanced join-card assets.",
    )
    group.add_command(_new_command("open", "Open the Welcome & Join control panel.", open_welcome_setup))
    group.add_command(_new_command("card-upload", "Upload custom join-card background artwork.", welcome_card_upload))
    group.add_command(_new_command("card-font-upload", "Upload a licensed custom join-card font.", welcome_card_font_upload))
    group.add_command(_new_command("card-font-clear", "Remove the uploaded join-card font.", welcome_card_font_clear))
    return group
...
def compact_public_dank_surface(bot: Any, tree: Any) -> int:
    ...
    if _COMPACTED:
        ...
        if size > DANK_PAYLOAD_SAFETY_LIMIT:
            raise RuntimeError(f"/dank command payload grew to {size}/{MAX_DANK_PAYLOAD} after compaction")
        return size
    ...
    dank_group.add_command(_new_command("home", "Open the Dank Shield control center.", open_dank_home))
    dank_group.add_command(_new_command("profile", "Open your profile signature settings.", open_profile_entry))
    dank_group.add_command(_new_command("help", "Show the short Dank Shield command guide.", open_help_entry))
    dank_group.add_command(_welcome_upload_group())

    _COMPACTED = True
    size = dank_payload_size(tree)
    children_after = sorted(str(getattr(command, "name", "")) for command in dank_group.commands)
    print(f"✅ public_command_hub compacted /dank payload={size}/{MAX_DANK_PAYLOAD} children={children_after}")
    if size > DANK_PAYLOAD_SAFETY_LIMIT:
        raise RuntimeError(
            f"/dank command payload is {size}/{MAX_DANK_PAYLOAD}; UI-first safety limit is {DANK_PAYLOAD_SAFETY_LIMIT}"
        )
    return size
