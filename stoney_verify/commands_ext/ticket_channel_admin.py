...
def _access_overwrite(*, can_send: bool) -> discord.PermissionOverwrite:
...
def register_ticket_channel_admin_commands(bot, tree) -> None:
    ...
    async def ticket_add(
        ...
        try:
            await ch.set_permissions(
                member,
                overwrite=_access_overwrite(can_send=True),
    ...
    async def ticket_remove(
        ...
        if owner and int(getattr(owner, "id", 0) or 0) == int(member.id):
            return await reply_once(interaction, {"content": "❌ You cannot remove the ticket owner. Transfer ownership first if needed.", "ephemeral": True})
    ...
    async def ticket_lock(
        ...
        try:
            await ch.set_permissions(
                owner,
                overwrite=_access_overwrite(can_send=False),
    ...
    async def ticket_owner(
        ...
        embed.add_field(name="Owner", value=f"{getattr(owner, 'mention', _safe_str(owner))}\n`{getattr(owner, 'id', 'unknown')}`", inline=False)
