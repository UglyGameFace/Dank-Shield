...
def _member_option_description(member: discord.Member) -> str:
    ...
    joined_text = joined.strftime("joined %Y-%m-%d") if joined is not None else "join date unknown"
    status = "bot" if member.bot else ("timed out" if member.is_timed_out() else "member")
...
class BulkMemberSelect(discord.ui.Select):
    def __init__(self, browser: "RoleMemberBrowserView") -> None:
        ...
        if not options:
            options = [discord.SelectOption(label="No members on this page", value="none")]
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        selected = [member for member in members if isinstance(member, discord.Member)]
        if not selected:
            await reply_ephemeral(interaction, "❌ None of the selected members are still available.")
            return
        view = BulkActionView(self.browser, selected)
        embed = discord.Embed(
            title="🧰 Safe Bulk Actions",
            description=(
                f"Selected **{len(selected)}** member(s) from {self.browser.role.mention}.\n\n"
        ...
        embed.add_field(
            name="Selected",
            value=trim("\n".join(f"• {display_name(member)} (`{member.id}`)" for member in selected), 1024),
            inline=False,
        )
...
class BulkActionView(OwnedView):
    def __init__(self, browser: "RoleMemberBrowserView", members: list[discord.Member]) -> None:
        self.browser = browser
        self.members = members
        super().__init__(browser.owner_id)

    @discord.ui.button(label="Send Reminder", emoji="💬", style=discord.ButtonStyle.primary, row=0)
    async def reminder(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        _ = button
        await interaction.response.send_modal(BulkReminderModal(self))

    @discord.ui.button(label="Add Role", emoji="➕", style=discord.ButtonStyle.secondary, row=0)
    async def add_role(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        _ = button
        await interaction.response.edit_message(
            embed=discord.Embed(title="➕ Bulk Add Role", description=f"Choose one role to add to **{len(self.members)}** selected members.", color=discord.Color.blurple()),
    ...
    async def remove_role(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        _ = button
        await interaction.response.edit_message(
            embed=discord.Embed(title="➖ Bulk Remove Role", description=f"Choose one role to remove from **{len(self.members)}** selected members.", color=discord.Color.blurple()),
            view=BulkRoleActionView(self, action="remove_role"),
        )

    @discord.ui.button(label="Back to Roster", emoji="◀️", style=discord.ButtonStyle.secondary, row=1)
    async def back(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        _ = button
        self.browser.rebuild()
        await interaction.response.edit_message(embed=self.browser.render_embed(), view=self.browser)
...
class BulkRoleSelect(discord.ui.RoleSelect):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        for target in self.parent_view.parent.members:
            blockers = role_action_blockers(interaction.guild, actor, target, role, self.parent_view.action)
            if blockers:
                blocked += 1
                continue
            try:
