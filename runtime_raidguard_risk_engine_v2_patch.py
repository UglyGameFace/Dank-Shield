...
def _log(message: str) -> None:
    try:
        print(f"🧠 runtime_raidguard_risk_engine_v2 {message}")
    except Exception:
        pass
...
def _has_any(flags: Iterable[str], names: Iterable[str]) -> bool:
    flag_set = {str(f).strip().lower() for f in flags if str(f or "").strip()}
    return any(str(name).strip().lower() in flag_set for name in names)
...
def _build_buckets(profile: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    weak = set(_string_list(profile.get("weak_signals"), 50))
    ...
    # Future post-join watchers can feed these fields without changing this engine.
...
def _combine_scores(profile: Dict[str, Any], buckets: Mapping[str, Mapping[str, Any]]) -> Tuple[int, str, str, List[str]]:
    ...
    # Evidence-first scoring. Identity/behavior/raid can dominate. Weak shape
    # signals need corroboration before medium/high.
    if identity >= 90:
