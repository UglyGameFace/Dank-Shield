...
def test_exact_format_patch_replaces_local_custom_format_fallback() -> None:
    ...
    # After the local applier runs, the runtime file should no longer contain it.
    ...
    if "Custom Format could not open" not in STUDIO:
        assert "async def _guard_design_action" in STUDIO
        assert "design.exact.open." in STUDIO


def test_exact_format_studio_debt_or_applied_slice_is_recorded() -> None:
    ...
    if "async def _guard_design_action" in STUDIO:
        for action_name in (
            "design.exact.examples",
            "design.exact.save_preview",
            "design.exact.server_style",
