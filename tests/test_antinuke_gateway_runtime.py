...
def test_install_adds_gateway_listener_and_member_impact_slow_burn_budget() -> None:
    fake_bot = FakeBot()
    ...
    try:
        ...
        assert {"ban", "kick", "member_prune"}.issubset(anti_nuke._SLOW_BURN_ACTIONS)
        assert gateway.install_anti_nuke_gateway_runtime(fake_bot) is False
        assert len(fake_bot.listeners) == 1
    finally:
        anti_nuke._SLOW_BURN_ACTIONS = original_actions
...
def test_gateway_role_create_delegates_to_targeted_rollback_handler(monkeypatch) -> None:
    _reset_runtime_state()
    entry = FakeEntry(7003, "role_create")
    calls: list[int] = []

    async def fake_role_handler(_guild, found_entry):
        calls.append(int(found_entry.id))

    monkeypatch.setattr(gateway, "_handle_dangerous_role_create", fake_role_handler)
