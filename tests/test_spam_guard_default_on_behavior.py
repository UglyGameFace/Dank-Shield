...
def test_brand_new_guild_defaults_enabled_and_bootstraps_row(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = asyncio.run(spam_guard.get_spam_settings(101))
...
def test_missing_db_row_is_default_enabled_not_a_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(spam_guard, "_fetch_settings_sync", lambda _gid: ("ok", {}, "row_missing"))
    monkeypatch.setattr(spam_guard, "_upsert_settings_sync", lambda _payload: (False, "temporary_write_failure"))

    settings = asyncio.run(spam_guard.get_spam_settings(102))
...
def test_explicit_persisted_enabled_row_stays_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = asyncio.run(spam_guard.get_spam_settings(103))
...
def test_explicit_persisted_disabled_row_stays_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    row = {"guild_id": "104", "spam_blocker_enabled": False, "spam_mode": "timeout"}
    upserts: list[dict] = []
    monkeypatch.setattr(spam_guard, "_fetch_settings_sync", lambda _gid: ("ok", row, "row_found"))
    monkeypatch.setattr(spam_guard, "_upsert_settings_sync", lambda payload: (upserts.append(payload) or True, "upsert_ok"))

    settings = asyncio.run(spam_guard.get_spam_settings(104))
...
def test_temporary_database_read_failure_uses_last_known_explicit_choice(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = asyncio.run(spam_guard.get_spam_settings(105))
...
def test_explicit_disabled_choice_survives_restart_via_persisted_row(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    saved, persisted = asyncio.run(spam_guard.save_spam_settings(106, {"enabled": False}))
    ...
    reloaded = asyncio.run(spam_guard.get_spam_settings(106))
